...
import com.soict.hoangviet.procoroutines.data.network.ApiConstant;
import com.soict.hoangviet.procoroutines.data.network.ApiError;
import com.soict.hoangviet.procoroutines.data.network.api.ApiException;
import com.soict.hoangviet.procoroutines.data.network.api.NetworkConnectionInterceptor;
...
import com.soict.hoangviet.procoroutines.custom.ViewController;
...
import com.soict.hoangviet.procoroutines.data.network.response.ObjectLoadMoreResponse;
...
public abstract class BaseFragment<T extends ViewDataBinding> extends DaggerFragment {
    ...
    protected void handleListResponse(ListResponse<?> response) {
        switch (response.getType()) {
            ...
            case Define.ResponseStatus.SUCCESS:
                getListResponse(response.getResponse(), response.isRefresh(), response.isLoadingMore());
                hideLoading();
                break;
    ...
    protected <U> void handleListLoadMoreResponse(ListLoadMoreResponse<U> response) {
        switch (response.getType()) {
            ...
            case Define.ResponseStatus.SUCCESS:
                hideLoading();
                getListResponse(response.getResponse().getNews(), response.isRefresh(), response.isLoadingMore());
                break;
            case Define.ResponseStatus.ERROR:
                hideLoading();
                handleNetworkError(response.getError());
